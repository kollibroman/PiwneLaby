...
export default function GameForm({ game, onSubmit, isSubmitting = false }: GameFormProps) {
    ...
    const inputClass = (field: keyof GameFormData) =>
        `w-full px-4 py-3 border-2 rounded-lg transition-colors ${showError(field) ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-cyan-600'
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-3">
                    {CATEGORIES.map(category => (
                        <label
                            key={category}
                            className={`flex items-center gap-2 px-4 py-2 border-2 rounded-lg cursor-pointer transition-colors ${formData.category.includes(category)
